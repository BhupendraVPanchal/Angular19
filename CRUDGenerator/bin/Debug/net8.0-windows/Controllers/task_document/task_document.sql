...
CREATE  VIEW [dbo].adv_task_document_select AS
SELECT
    t0.task_documentid AS task_documentid,
    t0.taskid AS taskid,
    t0.document_name AS document_name,
    t0.document_description AS document_description,
    ...
    t0.deleted_by AS deleted_by  FROM dbo.task_document t0
...
CREATE proc [dbo].[adp_task_document_select]
(
...
WHERE c1.OBJECT_ID = OBJECT_ID('dbo.task_document')  
Union All 
Select 'Edit', 200, -1 
Union All 
Select 'Delete', 201, -1 
...
@sort_type,--@sort_type 
'[dbo].adv_task_document_select',--@table_name 
'*',--@select_columns 
'' 
...
CREATE PROCEDURE [dbo].adp_task_document_insert_or_update
    @task_documentid INT,
    @taskid INT,
...
BEGIN TRAN
 ...
 IF(@task_documentid = 0)
BEGIN
Select @task_documentid = ISNULL(MAX(task_documentid), 0) + 1 from [dbo].task_document
    INSERT INTO [dbo].task_document (
        task_documentid,
        taskid,
        document_name,
...
update_count = ISNULL(update_count, 0) + 1
    ...
    WHERE task_documentid = @task_documentid;
END
Select * from [dbo].task_document Where task_documentid= @task_documentid
COMMIT TRAN
END TRY
BEGIN CATCH
ROLLBACK TRAN
...
CREATE PROCEDURE [dbo].adp_task_document_read
    @task_documentid INT
AS
BEGIN
SET NOCOUNT ON
...
CREATE PROCEDURE [dbo].adp_task_document_delete
    @task_documentid INT,
    @login_code INT
AS
BEGIN
