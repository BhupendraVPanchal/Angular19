...
import { Task_DocumentAddEditComponent}  from './task_document-add-edit/task_document-add-edit.component';
...
@Component({
selector: 'app-task_document',
standalone: false,
templateUrl: './task_document.component.html',
styleUrls: ['./task_document.component.scss']
})
export class Task_DocumentComponent implements OnInit, AfterViewInit {
...
grid_config: any = { 
title:'<h5 class="mb-0">task_document</h5>', 
grid_name: 'task_document', 
is_addbtn: false, 
full_width: true, 
grid_class: '', 
data_count:1000 
...
tablename: '[dbo].task_document ', 
primarykeycolumn: 'task_documentid', 
primarykeyvalue: '0', 
...
page_number: 1, 
page_size: 20, 
search_column: 'task_documentid', 
search_text: '', 
sort_col_name: 'task_documentid', 
sort_type: 'Desc' 
}; 
this.data_items = []; 
} 
...
this._masterserviceobj.get_task_document_data(this.query) 
.subscribe({ 
next: (RtnData) => { 
...
let dialogRef = this._dialog.open(Task_DocumentAddEditComponent, { data: { entity_code: _entity_code }, panelClass: 'mid-width' }); 
...
if (r && r.isConfirmed) 
{ 
this._masterserviceobj.delete_task_document({ task_documentid: _code }).subscribe({ 
next: (data) => { 
this.data_items = this.data_items.filter((x) => { 
return !(x.task_documentid == _code) 
}) 
}, error: (err_response) => { 
...
link.download = this._title + ".xlsx"; 
link.click(); 
...
{ 
this.add_edit_record(event.data.task_documentid); 
} 
else if (ColumnCaption == 'delete') 
{ 
this.delete_record(event.data.task_documentid); 
} 
