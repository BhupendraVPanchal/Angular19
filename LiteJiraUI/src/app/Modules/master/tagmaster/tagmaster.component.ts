...
import { TagmasterAddEditComponent } from './tagmaster-add-edit/tagmaster-add-edit.component';
...
@Component({
  ...
  styleUrls: ['./tagmaster.component.scss']
})
export class TagmasterComponent implements OnInit, AfterViewInit {
  public _headertitle: string = 'Tag Master';
  ...
  grid_config: any = {
    title: '<h5 class="mb-0">Tag Master</h5>',
    grid_name: 'tagmaster',
    is_addbtn: false,
    full_width: true,
    grid_class: '',
  ...
  table_info: any = {
    ...
    tablename: '[dbo].tagmaster ',
    primarykeycolumn: 'tagid',
    primarykeyvalue: '0',
  ...
  public get_master_data(): void {
    this._masterserviceobj.get_tagmaster_data(this.query)
      .subscribe({
        next: (RtnData) => {
  ...
  public add_edit_record(_entity_code: string): void {
    let dialogRef = this._dialog.open(TagmasterAddEditComponent, { data: { entity_code: _entity_code }, panelClass: 'mid-width' });
  ...
  public delete_record(_code: string): void {
    this._dynamicalertservice.fire({
      ...
      if (r && r.isConfirmed) {
        this._masterserviceobj.delete_tagmaster({ tagid: _code }).subscribe({
          next: (data) => {
            this.data_items = this.data_items.filter((x) => {
              return !(x.tagid == _code)
            })
          }, error: (err_response) => {
  ...
  public custom_events(event: any) {
    ...
    if (ColumnCaption == 'edit') {
      this.add_edit_record(event.data.tagid);
    }
    else if (ColumnCaption == 'delete') {
      this.delete_record(event.data.tagid);
    }
