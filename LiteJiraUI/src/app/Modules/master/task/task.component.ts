...
export class TaskComponent implements OnInit, AfterViewInit {
  public _headertitle: string = 'To-do';
  ...
  public init() {
    ...
    this.query = {
      page_number: 1,
      page_size: 20,
      search_column: 'taskid',
      search_text: '',
      sort_col_name: 'taskid',
      sort_type: 'Desc'
    };
    this.data_items = [];
  }
  ...
  public get_master_data(): void {
    this._masterserviceobj.get_task_data(this.query)
      .subscribe({
        next: (RtnData) => {
  ...
  public add_edit_record(_entity_code: string): void {
    let dialogRef = this._dialog.open(TaskAddEditComponent, { data: { entity_code: _entity_code }, panelClass: 'mid-width' });
  ...
  public delete_record(_code: string): void {
    this._dynamicalertservice.fire({
      ...
      if (r && r.isConfirmed) {
        this._masterserviceobj.delete_task({ taskid: _code }).subscribe({
          next: (data) => {
            this.data_items = this.data_items.filter((x) => {
              return !(x.taskid == _code)
            })
          }, error: (err_response) => {
  ...
  public custom_events(event: any) {
    ...
    if (ColumnCaption == 'edit') {
      this.add_edit_record(event.data.taskid);
    }
    else if (ColumnCaption == 'delete') {
      this.delete_record(event.data.taskid);
    }
