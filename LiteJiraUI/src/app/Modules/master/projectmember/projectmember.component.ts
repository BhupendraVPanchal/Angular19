...
@Component({
  ...
  styleUrls: ['./projectmember.component.scss']
})
export class ProjectmemberComponent implements OnInit, AfterViewInit {
  public _headertitle: string = 'Project Member';
  ...
  grid_config: any = {
    title: '<h5 class="mb-0">Project Member</h5>',
  ...
  public get_master_data(): void {
    this._masterserviceobj.get_projectmember_data(this.query)
      .subscribe({
        next: (RtnData) => {
  ...
  public delete_record(_code: string): void {
    this._dynamicalertservice.fire({
      ...
      if (r && r.isConfirmed) {
        this._masterserviceobj.delete_projectmember({ projectmemberid: _code }).subscribe({
          next: (data) => {
            this.data_items = this.data_items.filter((x) => {
              return !(x.projectmemberid == _code)
            })
          }, error: (err_response) => {
  ...
  public custom_events(event: any) {
    ...
    if (ColumnCaption == 'edit') {
      this.add_edit_record(event.data.projectmemberid);
    }
    else if (ColumnCaption == 'delete') {
      this.delete_record(event.data.projectmemberid);
    }
